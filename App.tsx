    ...
    <div className="min-h-screen scanlines bg-[#050505] text-slate-200 p-4 md:p-8 overflow-x-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-12 text-center space-y-4">
          <h1 className="text-5xl md:text-7xl font-bold tracking-tighter text-teal-500" style={{ textShadow: '4px 4px 0px rgba(20, 184, 166, 0.2)' }}>
            PROMPTCLOAK
          </h1>
        ...
        <nav className="flex flex-wrap justify-center gap-4 mb-12">
          <button 
            ...
            className={`px-8 py-2 border-b-4 transition-all uppercase text-xl font-bold ${mode === AppMode.ENCRYPT ? 'border-teal-500 text-teal-400 bg-teal-900/20' : 'border-slate-800 text-slate-600 hover:text-slate-400'}`}
          >
            [ Encrypt ]
          </button>
          <button 
            onClick={() => setMode(AppMode.DECRYPT)}
            className={`px-8 py-2 border-b-4 transition-all uppercase text-xl font-bold ${mode === AppMode.DECRYPT ? 'border-teal-500 text-teal-400 bg-teal-900/20' : 'border-slate-800 text-slate-600 hover:text-slate-400'}`}
          >
            [ Decrypt ]
          </button>
          <button 
            onClick={() => setMode(AppMode.QR_TOOLS)}
            className={`px-8 py-2 border-b-4 transition-all uppercase text-xl font-bold ${mode === AppMode.QR_TOOLS ? 'border-teal-500 text-teal-400 bg-teal-900/20' : 'border-slate-800 text-slate-600 hover:text-slate-400'}`}
          >
            [ QR Tools ]
          </button>
        </nav>
